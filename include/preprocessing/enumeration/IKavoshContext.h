...
struct IKavoshContext
{
     ...
     */
    SGF_HD IKavoshContext(const int64_t run_id, const uint32_t root,
                          const MotifCanonical* canonical, const uint32_t canonical_size) noexcept
        : m_run_id(run_id)
        , m_root(root)
        , m_canonical(canonical)
        , m_canonical_size(canonical_size)
    {
    }
    ...
    IKavoshContext(const IKavoshContext&) = delete;
    IKavoshContext& operator=(const IKavoshContext&) = delete;
    IKavoshContext(IKavoshContext&&) = delete;
    IKavoshContext& operator=(IKavoshContext&&) = delete;
