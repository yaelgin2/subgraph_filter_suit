    ...
    const std::vector<std::pair<double, uint32_t>>& scored_states)
{
    ...
    if ((state_count < MIN_STATES_FOR_GAP_PRUNE) || (minimum_keep_count >= state_count) ||
        (score_range <= 0.0))
    {
        return state_count;
    }
...
{
    return std::any_of(m_beam.begin(), m_beam.end(),
                       [this, threshold](const PatternState& state)
                       {
...
/* ---------- collect_sorted_colors_with_matches ---------- */

std::vector<std::tuple<double, uint32_t, uint32_t>>
SingleGraphPatternFinder::collect_sorted_colors_with_matches(
        ...
        if (is_absent_or_zero)
        {
            const double absent_prob =
                (color_id < static_cast<uint32_t>(m_color_probability.size()))
                    ...
                    : 0.0;
            return {
                {absent_prob, color_id, static_cast<uint32_t>(vertices_by_color[color_id].size())}};
        }
        sorted_colors.emplace_back(m_color_probability[color_id], color_id,
...
void SingleGraphPatternFinder::build_initial_beam(const ColoredGraph& search_graph)
{
    ...
    const std::vector<SeedInfo> seeds =
        select_valid_seeds(sorted_colors, vertices_by_color, initial_beam_size);
    if (seeds.empty())
    {
        m_beam.clear();
        return;
