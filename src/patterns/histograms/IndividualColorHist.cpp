    ...
    const uint32_t current_vertex_color, const std::vector<uint32_t>& neighbour_depths)
{
    ...
    SGF_DEBUG_LOG(m_logger, "BEFORE individual: " + format_hist_matrix(m_number_of_neighbours));
    ...
    SGF_DEBUG_LOG(m_logger, "AFTER individual: " + format_hist_matrix(m_number_of_neighbours));
}
...
{
    SGF_DEBUG_LOG(m_logger, "remove_node: depth=" + std::to_string(remove_node_depth) +
                                " neighbour_colors=" + format_uint_vector(neighbour_colors));
    SGF_DEBUG_LOG(m_logger, "BEFORE individual: " + format_hist_matrix(m_number_of_neighbours));
