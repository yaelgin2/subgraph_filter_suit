...
SingleGraphHistogram::SingleGraphHistogram(  // NOLINT(readability-function-size)
    const ColoredGraph& search_graph, const std::vector<double>& vertex_color_probabilities,
    double background_log_density, double initial_alpha_weight, double alpha_decay_rate,
    ...
    for (uint32_t color_index = 0; color_index < vertex_color_probabilities.size(); ++color_index)
    {
        ...
        m_color_log_probabilities[color_index] =
            (vertex_color_probabilities[color_index] > 0.0)
                ...
                : std::numeric_limits<double>::lowest() * half_factor;
    }
}
...
{
    const uint32_t vertex_color = static_cast<uint32_t>(m_graph.get_vertex_color(search_vertex));
    if (vertex_color >= m_color_log_probabilities.size())
    {
        throw std::out_of_range("SingleGraphHistogram::log_prob_of_vertex: color " +
...
double SingleGraphHistogram::log_prob_of_color(uint32_t remapped_color_id) const
{
    ...
    {
        throw std::out_of_range("SingleGraphHistogram::log_prob_of_color: color " +
...
void SingleGraphHistogram::add_all_vertex_neighbours_to_candidate(uint32_t absorbed_vertex,
                                                                  bool is_reversed)
{
    auto [neighbor_begin, neighbor_end] = m_graph.get_neighbours(absorbed_vertex, is_reversed);
    for (auto neighbor_iterator = neighbor_begin; neighbor_iterator != neighbor_end;
         ++neighbor_iterator)
    {
...
void SingleGraphHistogram::add_vertex_neighbour_to_candidate(uint32_t candidate_vertex,
                                                             uint32_t absorbed_vertex)
{
...
std::optional<double>
SingleGraphHistogram::compute_candidate_score(uint32_t candidate_vertex,
                                              double current_alpha_weight) const
{
    ...
    const uint32_t match_neighbor_count =
        (degree_iterator != m_candidate_match_neighbor_count.end()) ? degree_iterator->second : 0U;
    ...
    const double candidate_score =
        ...
        log_prob_of_vertex(candidate_vertex) + current_alpha_weight * outside_logp_iterator->second;

    if (std::isinf(candidate_score))
    {
        throw HistogramOverflowException("candidate score reached infinity for vertex " +
                                         std::to_string(candidate_vertex));
    }
...
void select_top_k_candidates(std::vector<sgf::CandidateVertex>& candidates, uint32_t result_count)
{
...
std::vector<CandidateVertex> SingleGraphHistogram::get_top_k_vertices(uint32_t max_result_count)
{
    ...
    const double current_alpha_weight =
        m_initial_alpha_weight * std::pow(m_alpha_decay_rate, static_cast<double>(m_current_depth));

    std::vector<CandidateVertex> scored_candidates;
    scored_candidates.reserve(m_candidates.size());

    for (const uint32_t candidate_vertex : m_candidates)
    {
