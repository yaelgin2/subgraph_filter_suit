 ...
 */
double PatternScorer::score(double pattern_vertex_color_log_prob, uint32_t pattern_edge_count,
                            double background_edge_density, uint32_t vertex_count)
{
    ...
    const uint64_t potential_pattern_edges =
        static_cast<uint64_t>(vertex_count) * static_cast<uint64_t>(vertex_count - 1) / 2ULL;
    ...
    const double background_edge_probability = clamp_probability(background_edge_density);
    ...
    const double edge_log_probability =
        static_cast<double>(pattern_edge_count) * std::log(background_edge_probability) +
