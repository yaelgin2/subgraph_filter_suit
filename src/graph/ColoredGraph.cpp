...
void ColoredGraph::fill_index_range(const uint32_t from_vertex,
                                  ...
                                  std::vector<uint32_t>& index_of_neighbours)
{
    ...
    for (const auto& edge : edges)
    {
        ...
        {
            fill_index_range(current_vertex, edge.first,
                             static_cast<uint32_t>(neighbours.size()),
                             index_of_neighbours);
    ...
    fill_index_range(current_vertex, num_vertices - 1,
                     static_cast<uint32_t>(neighbours.size()),
                     index_of_neighbours);
}
                                 ...
                                 const uint32_t num_vertices)
{
    ...
    {
        throw InvalidArgumentException(
            "self-loop at vertex " + std::to_string(edge.first) + " is not allowed");
    }
