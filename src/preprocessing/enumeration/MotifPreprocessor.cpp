    ...
    const GroupCounterCallback& count_group)
{
    ...
    {
        stream_groups_to_counter_for_vertex(graph_adjacency_matrix, count_group, processed_vertices, bfs_visited, vertex);
    ...
    uint32_t root)
{
    ...
    auto [first_degree_neighbours_begin, first_degree_neighbours_end] = m_graph.get_neighbours(root);
    for (auto first_degree_neigbour = first_degree_neighbours_begin; 
        first_degree_neigbour != first_degree_neighbours_end;
        ++first_degree_neigbour)
    {
        bfs_visited_vertices[*first_degree_neigbour] = run_id + 1; // mark as visited with depth 1
    }
    for (auto first_neighbour = first_degree_neighbours_begin; first_neighbour != first_degree_neighbours_end; ++first_neighbour)
    {
        if (visited_vertices_to_ignore[*(first_neighbour)])
        {
            continue;
        }
        for (auto second_neighbour = first_neighbour+1; second_neighbour != first_degree_neighbours_end; ++second_neighbour)
        {
            if (visited_vertices_to_ignore[*(second_neighbour)])
            {
                continue;
            }
            for (auto third_neighbour = second_neighbour+1; third_neighbour != first_degree_neighbours_end; ++third_neighbour)
            {
                if (visited_vertices_to_ignore[*(third_neighbour)])
                {
                    continue;
                }
                std::vector<uint32_t> group = {root, *(first_neighbour), *(second_neighbour), *(third_neighbour)};
    ...
    for (auto first_neighbour = first_degree_neighbours_begin; first_neighbour != first_degree_neighbours_end; ++first_neighbour)
    {
        ...
        auto[second_degree_neighbours_begin, second_degree_neighbours_end] = m_graph.get_neighbours(*first_neighbour);
        for (auto second_degree_neighbour = second_degree_neighbours_begin; 
            second_degree_neighbour != second_degree_neighbours_end;
             ++second_degree_neighbour)
        {
            if ((bfs_visited_vertices[*second_degree_neighbour] >> 2) != root)
            {
                bfs_visited_vertices[*second_degree_neighbour] = run_id + 2; // mark as visited with depth 2
            }
        }
             ...
             ++second_degree_neighbour)
        {
                ...
                ++second_first_degree_negihbour)
            {
                ...
                bool edge_exists = graph_adjacency_matrix[*(second_first_degree_negihbour)][*(second_degree_neighbour)] ||
                        graph_adjacency_matrix[*(second_degree_neighbour)][*(second_first_degree_negihbour)];
                ...
                {
                    std::vector<uint32_t> group = {root, *(first_neighbour), *(second_first_degree_negihbour), *(second_degree_neighbour)};
                    count_group(compute_motif_descriptor(group, graph_adjacency_matrix), group);
                }
            }
        }
            ...
            ++first_second_degree_neighbour)
        {
                ...
                ++second_second_degree_neighbour)
            {
                ...
                std::vector<uint32_t> group = {root, *(first_neighbour), *(first_second_degree_neighbour), *(second_second_degree_neighbour)};
                count_group(compute_motif_descriptor(group, graph_adjacency_matrix), group);
            }
        }
    }
    ...
    for (auto first_degree_neighbour = first_degree_neighbours_begin; first_degree_neighbour != first_degree_neighbours_end; ++first_degree_neighbour)
    {
        if (visited_vertices_to_ignore[*(first_degree_neighbour)])
        {
            continue;
        }
        auto[second_degree_neighbours_begin, second_degree_neighbours_end] = m_graph.get_neighbours(*first_degree_neighbour);
        for (auto second_degree_neighbour = second_degree_neighbours_begin; 
            second_degree_neighbour != second_degree_neighbours_end;
             ++second_degree_neighbour)
        {
            ...
            auto[third_degree_neighbours_begin, third_degree_neighbours_end] = m_graph.get_neighbours(*second_degree_neighbour);
            for (auto third_degree_neighbour = third_degree_neighbours_begin;
                third_degree_neighbour != third_degree_neighbours_end;
                ++third_degree_neighbour)
            {
                ...
                {
                    bfs_visited_vertices[*third_degree_neighbour] = run_id + 3; // mark as visited with depth 3
                    std::vector<uint32_t> group = {root, *(first_degree_neighbour), *(second_degree_neighbour), *(third_degree_neighbour)};
                    count_group(compute_motif_descriptor(group, graph_adjacency_matrix), group);
                }
                else if (bfs_visited_vertices[*third_degree_neighbour] == (run_id + 1))
                {
                    continue;
                         ...
                         !(graph_adjacency_matrix[*(first_degree_neighbour)][*third_degree_neighbour] ||
                         graph_adjacency_matrix[*third_degree_neighbour][*(first_degree_neighbour)]))
                {
                    std::vector<uint32_t> group = {root, *(first_degree_neighbour), *(second_degree_neighbour), *(third_degree_neighbour)};
                    count_group(compute_motif_descriptor(group, graph_adjacency_matrix), group);
                }
                                                       ...
                                                       const std::vector<uint32_t>& node_colors)
{
    ...
    __uint128_t minimal_colors = 0;
    const std::unordered_map<uint32_t, MotifCanonical>& motif_map = m_graph.is_directed()
             ? DIRECTED_MOTIF_CANONICAL_MAP : UNDIRECTED_MOTIF_CANONICAL_MAP;
    ...
    for(std::array<uint32_t, SgfConstants::MOTIF_SIZE> color_permutation : motif_canonical.color_permutations)
    {
        __uint128_t color_permutation_number = 0;
        for (size_t color_index = 0;  color_index < SgfConstants::MOTIF_SIZE; ++color_index)
        {
            color_permutation_number += node_colors[color_permutation[color_index]] << ((color_index) * SgfConstants::BITS_PER_COLOR);
            minimal_colors = std::min(minimal_colors, color_permutation_number);
        }
    }
    canonical_motif_num = (minimal_motif_num << (SgfConstants::MOTIF_SIZE * SgfConstants::BITS_PER_COLOR)) | minimal_colors;
    ...
    const std::vector<std::vector<bool>>& graph_adjacency_matrix) const
{
    ...
    for (size_t row_index = 0; row_index < SgfConstants::MOTIF_SIZE; ++row_index)
    {
        ...
        for (; column_index < SgfConstants::MOTIF_SIZE; ++column_index)
        {
            ...
            motif_descriptor <<= 1;
            motif_descriptor += graph_adjacency_matrix[group[row_index]][group[column_index]]? 1 : 0;
        }
    }
