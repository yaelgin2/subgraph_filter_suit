                                          ...
                                          IOConstants::GraphmlDirectedBoostGraph& out)
{
    ...
    {
        const auto new_vertex = boost::add_vertex(out);
                                       ...
                                       IOConstants::GraphmlDirectedBoostGraph& out)
{
    ...
    {
        const auto [first, second] = boost::out_edges(src, graph);

        for (auto it = first; it != second; ++it)
        {
            const auto edge = *it;
            const auto dst = boost::target(edge, graph);

            const auto [e, ok] = boost::add_edge(src, dst, out);
            static_cast<void>(ok);
