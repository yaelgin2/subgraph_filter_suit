                                   ...
                                   const bool is_directed) const
{
    ...
    {
        const std::string pattern_path = build_pattern_full_path(pattern_idx, timestamp, *writer);
        writer->write(result.first, pattern_path, m_logger, is_directed);
        mapping.emplace(std::filesystem::path(pattern_path).filename().string(), result.second);
        ++pattern_idx;
    }
