                                                   ...
                                                   const uint8_t format32)
{
    if (size <= MSGPACK_FIX_COLLECTION_MAX)
    {
        ...
        output_stream.write(&byte, SINGLE_BYTE);
    }
    else if (size <= MSGPACK_COLLECTION16_MAX)
    {
        ...
        const std::array<char, UINT16_MSGPACK_BYTE_COUNT> bytes = {
            ...
            static_cast<char>((static_cast<uint16_t>(len >> SHIFT_8)) & BYTE_MASK),
            static_cast<char>(len & BYTE_MASK)};
    ...
    else
    {
        ...
        const std::array<char, UINT32_MSGPACK_BYTE_COUNT> bytes = {
            static_cast<char>(format32), static_cast<char>((len >> SHIFT_24) & BYTE_MASK),
            static_cast<char>((len >> SHIFT_16) & BYTE_MASK),
            static_cast<char>((len >> SHIFT_8) & BYTE_MASK), static_cast<char>(len & BYTE_MASK)};
        output_stream.write(bytes.data(), static_cast<std::streamsize>(bytes.size()));
    }
}
...
void BinaryCacheIOManager::write_uint32_value(std::ofstream& output_stream, const uint32_t value)
{
    const std::array<char, UINT32_MSGPACK_BYTE_COUNT> bytes = {
        ...
        static_cast<char>((value >> SHIFT_24) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_16) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_8) & BYTE_MASK), static_cast<char>(value & BYTE_MASK)};
    output_stream.write(bytes.data(), static_cast<std::streamsize>(bytes.size()));
}
...
void BinaryCacheIOManager::write_uint64_value(std::ofstream& output_stream, const uint64_t value)
{
    const std::array<char, UINT64_MSGPACK_BYTE_COUNT> bytes = {
        ...
        static_cast<char>((value >> SHIFT_56) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_48) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_40) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_32) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_24) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_16) & BYTE_MASK),
        static_cast<char>((value >> SHIFT_8) & BYTE_MASK),
        static_cast<char>(value & BYTE_MASK)};
    output_stream.write(bytes.data(), static_cast<std::streamsize>(bytes.size()));
}
...
{
    std::array<char, UINT32_BYTE_IDX_2> buf{};
    input_stream.read(buf.data(), static_cast<std::streamsize>(buf.size()));
...
{
    const uint8_t byte = read_byte(input_stream);
    if (byte == format32)
    {
