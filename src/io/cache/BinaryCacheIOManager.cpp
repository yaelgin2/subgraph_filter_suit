                                                   ...
                                                   const uint8_t format32)
{
    ...
    {
        const char raw_byte =
            static_cast<char>(fix_base | (static_cast<uint8_t>(size) & MSGPACK_FIX_COLLECTION_MASK));
    ...
    else if (size <= MSGPACK_COLLECTION16_MAX)
    {
        ...
        const std::array<char, UINT16_MSGPACK_BYTE_COUNT> bytes = {
            ...
            static_cast<char>((static_cast<uint32_t>(len) >> SHIFT_8) & BYTE_MASK),
...
void BinaryCacheIOManager::check_read_stream(const std::ifstream& input_stream)
{
    ...
    {
        throw GraphConstructionException("Unexpected end of data or read error in binary cache file");
    }
}
