...
po::options_description SgfGraphEnumeratorArgumentParser::build_options()
{
    ...
    feature_desc.add_options()(KEY_MOTIFS, po::bool_switch(),
                                "Enable motif-based processing")(
        KEY_PATHS, po::bool_switch(), "Enable path-based processing");

    po::options_description common_desc("Common flags");
    common_desc.add_options()(KEY_IS_DIRECTED, po::bool_switch(),
    ...
    preprocess_desc.add_options()(KEY_LIBRARY_DIR, po::value<std::string>(),
        ...
        KEY_CACHE_DIR, po::value<std::string>(),
        "Directory where the cache library is written");
    ...
    filter_desc.add_options()(KEY_GRAPH_DIR, po::value<std::string>(),
        ...
        "Graph file format: graphml, vertex-edge, json")(
        KEY_RESULT_FOLDER, po::value<std::string>(),
        "Directory for filter result output")(
        KEY_RESULT_TYPE, po::value<std::string>(),
        "Result format: json, csv")(
        KEY_MOTIF_CACHE_FILE, po::value<std::string>(),
...
po::variables_map SgfGraphEnumeratorArgumentParser::parse_raw(
...
PreprocessArgs
SgfGraphEnumeratorArgumentParser::parse_preprocess_args(const po::variables_map& vm)
{
...
FilterArgs SgfGraphEnumeratorArgumentParser::parse_filter_args(const po::variables_map& vm)
{
    ...
    result.m_graph_input_type =
        parse_reader_type(get_required_string(vm, KEY_GRAPH_INPUT_TYPE));
    ...
    result.m_load_motif_graph_cache_path =
        get_optional_string(vm, KEY_LOAD_MOTIF_GRAPH_CACHE);
    result.m_load_path_graph_cache_path =
        get_optional_string(vm, KEY_LOAD_PATH_GRAPH_CACHE);
    return result;
}
...
std::string
SgfGraphEnumeratorArgumentParser::get_required_string(const po::variables_map& vm,
...
std::string
SgfGraphEnumeratorArgumentParser::get_optional_string(const po::variables_map& vm,
                                                       const std::string& key)
{
    if (vm.count(key) == 0U)
    {
...
GraphReaderType
SgfGraphEnumeratorArgumentParser::parse_reader_type(const std::string& type_str)
{
...
CacheManagerType
SgfGraphEnumeratorArgumentParser::parse_cache_type(const std::string& type_str)
{
...
ResultOutputType
SgfGraphEnumeratorArgumentParser::parse_result_type(const std::string& type_str)
{
