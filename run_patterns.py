#!/usr/bin/env python3
"""
Find 100 patterns from the graph library in temp/input_color_uniform_deg_3/
and write each to temp/input_color_uniform_deg_3_patterns/pattern_{i}.json.

Patterns are found in parallel across NUM_WORKERS processes.
Each worker receives the graph list once (via initializer) and uses
pattern_idx as its random seed so every run produces a different pattern.
"""
...
INPUT_DIR  = Path(__file__).parent / "temp" / "input_color_rare_deg_3"
OUTPUT_DIR = Path(__file__).parent / "temp" / "input_color_rare_deg_3_patterns"
NUM_PATTERNS  = 200
START_IDX     = 0
...
def main() -> None:
    ...
    log_path = OUTPUT_DIR / "durations.log"
    ...
    print(f"Done — {NUM_PATTERNS} patterns written to {OUTPUT_DIR}")
