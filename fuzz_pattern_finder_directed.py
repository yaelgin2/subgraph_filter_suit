#!/usr/bin/env python3
"""Fuzz tester for the multigraph pattern finder — directed graphs.

For each (n, k) in [5..20] x [1..5], runs 1000 iterations in parallel:
  - Generates k random directed graphs of n vertices
  - Calls sgf-pattern-finder --is-directed with thresholds 1/k, 2/k, ..., k/k
  - Verifies via sgf-graph-searcher --directed and NetworkX DiGraphMatcher that
    the returned pattern is a subgraph in every graph the finder claims alive.

On failure the run directory is moved to fuzz_failure_directed/<run_name>/ and
the script stops.  run.log is written by sgf-pattern-finder via --log-file-path.
"""
...
FAILURE_BASE = _HERE / "fuzz_failure_directed"
...
def nx_is_subgraph(pattern_dict, graph_dict):
    ...
    return DiGraphMatcher(host, pat, node_match=node_match).subgraph_is_monomorphic()
...
def run_iteration(n, k, iteration):
    ...
    run_dir = Path(tempfile.mkdtemp(prefix=f"fuzz_dir_n{n}_k{k}_i{iteration}_"))
    ...
    graphs = [make_directed_graph(n, rng) for _ in range(k)]
    graph_paths = []
...
def main():
    ...
    progress = tqdm(total=len(tasks), unit="iter", desc="fuzzing directed")
    fail_dirs = []
