...
// Pointers to the device vectors declared above - no need to delete as the device_vectors live on
// the stack
...
{
    const char* motifVariations[4] = {undirected3, directed3, undirected4, directed4};
...
void GPUMotifCalculator::SetSortedNodes()
{
    ...
    //            nodesWithDegrees.push_back( { node, (unsigned
    //            int)(offsetList[root+1]-offsetList[root]) });
    //        }
...
void GPUMotifCalculator::CopyAllToDevice()
{
     ...
     */
    gpuErrchk(cudaDeviceSetLimit(cudaLimitMallocHeapSize, size_t(10) * size_t(numOfNodes) *
                                                              size_t(numOfNodes) * sizeof(int64)));
    size_t currentLimit;
    ...
    std::memcpy(this->devicePointerMotifVariations, &((*(this->nodeVariations))[0]),
                nodeVariations->size() * sizeof(unsigned int));
    // Removal index
    ...
    gpuErrchk(cudaMallocManaged(&(this->devicePointerSortedNodesByDegree),
                                sortedNodesByDegree->size() * sizeof(unsigned int)));
    std::memcpy(this->devicePointerSortedNodesByDegree, this->sortedNodesByDegree->data(),
                sortedNodesByDegree->size() * sizeof(unsigned int));
    ...
    // Original graph
    gpuErrchk(
        cudaMallocManaged(&deviceOriginalGraphOffsets, (this->numOfNodes + 1) * sizeof(int64)));
...
__global__ void Motif4Kernel()
{
    ...
    for (int i = index; i < n; i += stride)
    {
        ...
        {  // loop first neighbors
            ...
            for (int64 j = n1_idx + 1; j < offsets[root + 1]; j++)
            {
                if (j == offsets[root + 1] - 1)  // if j is the last element, we can't add an
                                                 // element and therefore it's not a 3-combination
                {
                    continue;
                }
            ...
            *//* Motifs of the form root, two first neighbors and a second neighbor (root-n1-n2-n11) */
            ...
            {  // loop second neighbors (again)
                ...
                for (int64 n11_idx = offsets[root]; n11_idx < offsets[root + 1]; n11_idx++)
                {  // loop first neighbors
                    ...
                    /* If there is an edge between n1 and n11, we would have already counted the
                     motif as a motif of depth 2. If no such edge exists, we only want to count the
                     motif once.  */
                    if (!((AreNeighbors(root, n2)) || (AreNeighbors(n2, root))) && n11 != n1)
                    {
                        bool edgeExists = AreNeighbors(n2, n11) || AreNeighbors(n11, n2);
            ...
            /* Motifs of the form root, a first neighbors and two second neighbors - The case of
             root-n1-n21-n22 2-combinations on second neighbors */
            for (int64 l = offsets[n1]; l < offsets[n1 + 1]; l++)
            {
                for (int64 j = l + 1; j < offsets[n1 + 1]; j++)
                {
                    ...
                    // If both nodes are not neighbors of root (were seen at level 2) then the motif
                    // wasn't counted as a depth 1 motif
                    if (!AreNeighbors(root, n21) && !AreNeighbors(n21, root) &&
                        !AreNeighbors(root, n22) && !AreNeighbors(n22, root))
                    {
            ...
            {  // loop second neighbors (third time's the charm)
                ...
                for (int64 n3_idx = offsets[n2]; n3_idx < offsets[n2 + 1]; n3_idx++)
                {
                    ...
                    /* According to the proved rules, check that n3 is not a first neighbor, not a
                    neighbor of the first neighbor n1 and not the neighbor n1 itself. If it is any
                    of them, continue */
                    if ((AreNeighbors(root, n3) || (AreNeighbors(n3, root))) ||
                        (AreNeighbors(n1, n3) || (AreNeighbors(n3, n1))) || (n1 == n3))
                    {
                        continue;
                    }
...
vector<vector<unsigned int>*>* GPUMotifCalculator::Calculate()
{
    ...
    {  // must sort before
        ...
        for (unsigned int node = 0; node < this->numOfNodes; node++)
        {
            ...
            nodesWithDegrees.push_back(
                {node, (unsigned int)(globalDeviceFullGraphOffsets[root + 1] -
                                      globalDeviceFullGraphOffsets[root])});
        }
        std::sort(nodesWithDegrees.begin(), nodesWithDegrees.end(), cmpNodesByDegree);
    ...
    //        unsigned int root= globalDevicePointerSortedNodesByDegree[i];
    //        std::cout<<"Node number "<<root<<" Deg =
    //        "<<globalDeviceFullGraphOffsets[root+1]-globalDeviceFullGraphOffsets[root]<<" removal
    //        index "<<globalDevicePointerRemovalIndex[root]<<"\n";
    //    }
    ...
    cudaMemPrefetchAsync(globalDeviceOriginalGraphNeighbors, neighborSize * sizeof(unsigned int),
                         device, NULL);
    ...
    cudaMemPrefetchAsync(globalDeviceFeatures,
                         (this->numOfNodes * this->numOfMotifs) * sizeof(unsigned int), device,
                         NULL);
    ...
    for (int node = 0; node < this->numOfNodes; node++)
    {
        ...
        {
            this->features->at(node)->at(motif) =
                globalDeviceFeatures[motif + this->numOfMotifs * node];
        }
    }
...
2. Motifs of depth 1
*/
__device__ void Motif3Subtree(unsigned int root)
{
    ...
    /* Since cacheGraph cannot be modified at runtime, we hold Removal Index,
        ...
        we will verify that the removal index of a node is lower than the removal index of the root
       of the current subtree. root_idx is also our current iteration */
...
*/
__device__ void Motif4Subtree(unsigned int root)
{
    ...
    for (int64 i = offsets[root]; i < end; i++)
    {
        ...
        {
            if (j == end - 1)  // if j is the last element, we can't add an element and therefore
                               // it's not a 3-combination
            {
                continue;
            }
    ...
    {  // loop first neighbors
        ...
        {  // loop second neighbors (again)
            ...
            {  // loop first neighbors
                ...
                /* If there is an edge between n1 and n11, we would have already counted the motif
                 as a motif of depth 2. If no such edge exists, we only want to count the motif
                 once.  */
                if (!((AreNeighbors(root, n2)) || (AreNeighbors(n2, root))) && n11 != n1)
                {
                    bool edgeExists = AreNeighbors(n2, n11) || AreNeighbors(n11, n2);
    ...
    {  // loop first neighbors
        ...
        {  // loop second neighbors (third time's the charm)
            ...
            for (int64 n3_idx = offsets[n2]; n3_idx < offsets[n2 + 1]; n3_idx++)
            {
                ...
                /* According to the proved rules, check that n3 is not a first neighbor, not a
                neighbor of the first neighbor n1 and not the neighbor n1 itself. If it is any of
                them, continue */
                if ((AreNeighbors(root, n3) || (AreNeighbors(n3, root))) ||
                    (AreNeighbors(n1, n3) || (AreNeighbors(n3, n1))) || (n1 == n3))
                {
...
__device__ void GroupUpdater(unsigned int group[], int size)
{
    ...
    if (motifNumber != -1)
    {
        ...
        {
            atomicAdd(globalDeviceFeatures + (motifNumber + globalNumOfMotifs * group[i]),
                      1);  // atomic add + access as 1D array : features[motif + M*node] //
...
GPUMotifCalculator::~GPUMotifCalculator()
{
    ...
    // the index in which we remove the node from the graph. Basically, from this index on the node
    // doesen't exist.
    delete removalIndex;
